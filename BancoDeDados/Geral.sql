...
create table escola.aluno(id int primary key not null identity(1,1), nome varchar(100), cpf varchar(100), data_nascimento datetime, ativo bit)
go
create table escola.professor(id int primary key not null identity(1,1), nome varchar(100), cpf varchar(100), ativo bit)
go
create table escola.curso(id int primary key not null identity(1,1), nome varchar(100), ativo bit)
go
create table escola.disciplina(id int primary key not null identity(1,1), nome varchar(100), ativo bit)
go
...
go
create table secretaria.pagamentos(id int primary key not null identity(1,1), boleto datetime, situacao varchar(100) ,id_aluno int foreign key references escola.aluno(id))
go
...
go
create table adm.log(id int primary key not null identity(1,1), tabela varchar(100), comando varchar(100), mensagem_de_erro varchar(100), data_erro date)
go
...
INSERT INTO escola.curso (nome,ativo) VALUES ('Kung�lv',1)
...
INSERT INTO escola.disciplina (nome,ativo) VALUES ('Tomasz�w Mazowiecki',1)
...
INSERT INTO escola.disciplina (nome,ativo) VALUES ('M�fil',0)
...
@nome varchar(max), @cpf varchar(max), @data_nascimento datetime
as
begin
	declare @idade int
...
--EXERCICIO07--
create nonclustered index IDX_BOLETO on secretaria.pagamentos (boleto) 
create nonclustered index IDX_CPF_ALUNO on escola.aluno (cpf) 
create nonclustered index IDX_CPF_PROFESSOR on escola.professor (cpf) 
create nonclustered index IDX_SITUACAO on secretaria.pagamentos (situacao) 
...
ALTER DATABASE atividade SET RECOVERY FULL;
go
exec insereAluno 'edjalma', '12345678998', '2002-12-12'
go
backup database atividade to disk='C:\backup\foi.bak' with init
go
exec insereAluno 'lemos', '65496848951', '2002-12-12'
go
backup database atividade to disk='C:\backup\foiDiferencial.bak' with differential
go
delete escola.aluno where nome = 'edjalma'
delete escola.aluno where nome = 'lemos'
go
backup log atividade to disk='C:\backup\foiLog.bak'